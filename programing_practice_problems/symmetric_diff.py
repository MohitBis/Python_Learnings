...
sets:
-----
If the inputs are given on one line separated by a character (the delimiter), use split() to get the separate values in the form of a 
list. The delimiter is space (ascii 32) by default. To specify that comma is the delimiter, use string.split(',').
...
m = set(map(int,m_elements))
n_size = int(input())
n_elements = input().split()
n = set(map(int,n_elements))
d1 = m.difference(n)
d2 = n.difference(m)
l = list(d1) + list(d2)
l.sort()
